...
if(isset($postdata) && !empty($postdata)){
    ...
    $queryHabitacionName = "SELECT nombre FROM habitacion WHERE idHabitacion = ?";
    ...
    $queryDeleteHabitacion = "DELETE FROM habitacion WHERE idHabitacion = ?";
    $stmt2 = $db->prepare($queryDeleteHabitacion);
    $stmt2->bind_param("i", $idHabitacion);
    ...
    if ($result) {
        ...
        $accion = "Se eliminó la habitación: " . $nombreCliente;
